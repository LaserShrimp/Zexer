...
void Game::start(SDL_Renderer *renderer) {
	score_ = 0;
	gameLoopActive_ = true;
	levelContinues_ = true;
	gamePaused_ = false;
...
// 	Mix_VolumeMusic(MIX_MAX_VOLUME);
    ...
    SDLMenu menu{};
    menu.setFont(TTF_OpenFont("Hybrid_b.ttf", 30));
    menu.addOption("hey !");
    menu.addOption("Resume");
    menu.addOption(("Leave the game"));
    menu.setCoordinates({WIN_WIDTH/2 - 50, WIN_HEIGHT/2 - 50, 100, 100});

    while (gameLoopActive_) {
        gameLoopActive_ = !inputs.getquit();
        if(inputs.getescape() && escapeReleased_) {
			gamePaused_ = true;
			escapeReleased_ = false;
        } else if(!inputs.getescape()) {
			escapeReleased_ = true;
		}
        if(player->getHealth() <= 0 && levelContinues_) {
            levelContinues_ = false;
            Mix_PlayChannel(-1, loseChunk, 0);
        }
        if(vEnemy.size() == 0 && levelContinues_) {
            w.increaseLevel();
// 			w.loadRandomizedLevel(vEnemy, w.getLevel());
            levelContinues_ = w.loadLevelFromFile(vEnemy, "level.lvl");
            if(!levelContinues_) {
...
void Game::pauseMenu(SDL_Renderer *renderer) {
    ...
    bool stayOnPause{true};
    while(stayOnPause) {
        SDL_PollEvent(&event);
        if(event.type == SDL_QUIT) {
            stayOnPause = false;
			gameLoopActive_ = false;
        }
        if(event.type == SDL_KEYUP) {
            ...
            case SDLK_s:
                ...
                break;
            case SDLK_RETURN: {
                std::string selection{menu.optionHovered()};
                if(selection == "Resume") {
                    menu.deinit();
					gamePaused_ = false;
// 					escapeReleased_ = true;
                    return;
                } else if(selection == "Leave game") {
                    gameLoopActive_ = false;
                    menu.deinit();
                    return;
                }
